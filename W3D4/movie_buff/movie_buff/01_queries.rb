...
def harrison_ford
  ...
  # the below 4 queries return the same result
  ...
  # SELECT movies.id, movies.title FROM movies JOIN castings ON castings.movie_id = movies.id JOIN actors ON actors.id = castings.actor_id WHERE actors.name = 'Harrison Ford' AND castings.ord > 1
