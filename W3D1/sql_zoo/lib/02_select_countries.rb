...
def high_population_gdps
  ...
  execute(<<-SQL)
  SELECT
    countries.name, (countries.gdp / countries.population) AS per_capita_gdp
  FROM
    countries
  WHERE
    countries.population >= 200000000;
...
def population_in_millions
  ...
  execute(<<-SQL)
  SELECT
    countries.name, (countries.population / 1000000) AS pop_millions
  FROM
   countries
  WHERE
